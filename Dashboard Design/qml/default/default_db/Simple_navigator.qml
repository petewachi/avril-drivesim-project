...
Item {
    ...
    Timer {
        ...
        property double latitude: simple_navigator.latitude
        property double longitude: simple_navigator.longitude
        running: simple_navigator.solver_status === dashboard_Utils.vICRT_SOLVER_STATUS_STARTED
                 && (trigger_refreash_map.latitude !== simple_navigator.latitude
                     || trigger_refreash_map.longitude !== simple_navigator.longitude)
        repeat: false
        triggeredOnStart: false
        interval: 1 * 1000
        onTriggered: {
            var temp=0;
            if((simple_navigator.latitude - trigger_refreash_map.latitude) > 0){
                temp = dashboard_Utils.rad2deg(
                            Math.atan((simple_navigator.longitude - trigger_refreash_map.longitude)
                                      / (simple_navigator.latitude - trigger_refreash_map.latitude)))
            }else{
            ...
            mapview.center = QtPositioning.coordinate(
                        simple_navigator.latitude, simple_navigator.longitude)
        }
    }
    ...
    Component.onCompleted: {
        ...
        simple_navigator.get_scenario_request();
    ...
    Timer{
        ...
        onTriggered:{
            ...
            if (simple_navigator.scenario_request.readyState === 4 &&
                    (Date.now() - timer_get_scenario_request.counter) < timeout*1000){
                simple_navigator.load_scenario()
                this.stop()
    ...
    function get_scenario_request () {
        ...
        simple_navigator.scenario_request = dashboard_Utils.openFileasync(
                    "http://".concat(simple_navigator.ws_ip, ":",
                                     simple_navigator.ws_port,
                                     "/worldsim/scenario/"))
        timer_get_scenario_request.start()
    }

    function load_scenario(){
        var ws_scenario_file = simple_navigator.scenario_request.responseText
        ...
        if (ws_scenario_file !== "") {
            ...
            if (ws_scenario_json["errorMessage"] !== undefined) {
                ...
                simple_navigator.ws_error = ws_scenario_json["errorMessage"]
                ...
                simple_navigator.ws_error = "Invalid scenario file!"
            }
        } else {
            //worldsim is not yet started
            simple_navigator.ws_error = "WorldSim not started at: "
                    + simple_navigator.ws_ip + ":" + simple_navigator.ws_port
        }

        simple_navigator.csv_logged_gps = simple_navigator.csv_logged_gps.concat(
                    ws_environment, ".csv")

        if (simple_navigator.ws_error !== "") {
            console.log("Loaded the default " + ws_environment
                        + " track because of failure of contacting VI-WorldSim")
            console.log(simple_navigator.ws_error)
        } else {
            console.log("Loading the " + ws_environment + " track ")
        }

        //loading text into a full string
        var allText = dashboard_Utils.openFile(simple_navigator.csv_logged_gps)
        console.log(simple_navigator.csv_logged_gps)
        ...
        if (index_latitude === -1 || index_longitude === -1) {
            ...
            simple_navigator.track_logged.push({
                                                   "latitude": 0,
                                                   "longitude": 0
                                               })
        } else {
        ...
        line_map.path = simple_navigator.track_logged
        trigger_refreash_map.latitude
                = simple_navigator.track_logged[simple_navigator.track_logged.length - 1].latitude
        trigger_refreash_map.longitude
                = simple_navigator.track_logged[simple_navigator.track_logged.length - 1].longitude
    ...
    Map {
        ...
        fieldOfView: 45
        opacity: simple_navigator.solver_status
        ...
        MapQuickItem {
            ...
            coordinate: QtPositioning.coordinate(simple_navigator.latitude,
                                                 simple_navigator.longitude)
            sourceItem: Image {
                id: image_back
                source: "images/navi_ego_back.png"
            }
            Timer {
                id: timer_mavigator
                property double increment: 0.01
                running: simple_navigator.solver_status
        ...
        Image {
            id: background_map
            source: "images/simple_navigator_back.png"
            anchors.fill: parent
        }
    }
    ...
    AnimatedImage {
        ...
        source: "images/waiting_gps.gif"
        anchors.horizontalCenter: parent.horizontalCenter
        fillMode: Image.PreserveAspectFit
        visible: simple_navigator.solver_status !== dashboard_Utils.vICRT_SOLVER_STATUS_STARTED
    }
